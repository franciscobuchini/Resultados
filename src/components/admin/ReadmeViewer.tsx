...
export default function ReadmeViewer() {
  ...
  const save = async (updated: Task[]) => {
    ...
    await supabase
      .from('apis')
      .upsert({ id: STORAGE_ID, data: updated, updated_at: new Date().toISOString() }, { onConflict: 'id' });
    setSaving(false);
  };
  ...
  const toggleStatus = async (id: string) => {
    const updated = tasks.map(t =>
      t.id === id ? { ...t, status: t.status === 'done' ? 'pending' : 'done' as Status } : t
    );
    setTasks(updated);
    await save(updated);
  };
    ...
    <div className={`max-w-3xl mx-auto px-4 py-8 flex flex-col gap-6 ${textMain}`}>
      ...
      <div className={`rounded-xl border ${border} ${bgSurface} p-4 flex flex-col gap-3`}>
        <div className="flex gap-2">
          <input
            ...
            className={`flex-1 bg-transparent text-sm outline-none placeholder:opacity-40 ${textMain}`}
          />
          {/* Selector de asignado */}
          <div className="flex gap-1">
            {(['FRAN', 'JAVI', 'AMBOS'] as Assignee[]).map(a => (
              <button
                key={a}
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-all disabled:opacity-30"
          >
            <Plus size={16} />
          </button>
        </div>
        ...
        <input
          ...
          onKeyDown={e => e.key === 'Enter' && addTask()}
          className={`bg-transparent text-xs outline-none placeholder:opacity-30 ${textMuted}`}
        />
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {filtered.map(task => (
          <div
            key={task.id}
            className={`group flex items-start gap-3 p-3.5 rounded-xl border ${border} ${bgSurface} transition-all hover:border-white/20 ${
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {task.note && (
                <p className={`text-xs mt-0.5 ${textMuted}`}>{task.note}</p>
              )}
            </div>
            ...
            <button
              ...
              className={`opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-red-500/20 hover:text-red-400 ${textMuted}`}
            >
              <Trash2 size={14} />
            </button>
          </div>
