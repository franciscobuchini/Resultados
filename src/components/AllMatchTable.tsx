...
interface AllMatchTableProps {
  utcOffset: number;
}

export default function AllMatchTable({ utcOffset }: AllMatchTableProps) {
  ...
  return (
    <div className="p-4 overflow-x-auto pb-24 text-center">
      <table className="w-full border-collapse text-[10px] font-mono bg-black text-zinc-400 min-w-max">
        <thead>
          <tr className="bg-neutral-950 sticky top-0">
            {columns.map(col => (
              <th key={col} className="border border-zinc-800 p-2 text-center text-sm text-zinc-100">{col}</th>
            ))}
          </tr>
        </thead>
        <tbody>
